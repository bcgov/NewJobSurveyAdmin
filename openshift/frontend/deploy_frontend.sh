...
try source ./deploy_frontend_vars.sh
echo ""
...
info "Updating OpenShift build from bc-frontend.yaml..."
try oc process -f ./bc-frontend.yaml -p NAME=$NAME IMAGE_TAG=$IMAGE_TAG TOOLS_NAMESPACE=$TOOLS_NAMESPACE | try oc apply -f -
echo ""
...
if [ "$DO_BUILD" = true ] ; then
  ...
  try oc start-build $NAME --from-dir=../../ClientApp/build --wait --follow
  echo ""
...
info "Updating OpenShift deployment from dc-frontend.yaml..."
try oc process -f ./dc-frontend.yaml -p NAME=$NAME API_NAME=$API_NAME \
  ...
  BASE_GATEWAY_URL=$BASE_GATEWAY_URL DEPLOY_ENV=$DEPLOY_ENV KEYCLOAK_ROLE=$KEYCLOAK_ROLE \
  | try oc apply -f -
echo ""
